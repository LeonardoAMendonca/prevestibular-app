...
import { Aluno, Operador } from "@/types";
...
import LoginOperador from "@/components/LoginOperador";
import { exportarLoteCompletoZip } from "@/lib/batch-exporter";


export const INITIAL_FORM_STATE: Partial<Aluno> = {
  // --- Dados Pessoais ---
  id: "",
  nome: "",
  dataNascimento: "",
  ...
  // --- Contatos de Emergência (Conforme última alteração) ---
  telefoneEmergencia1: "",
  nomeEmergencia1: "",
...
const STORAGE_KEY = "sistema_matricula";
...
export default function Home() {
  ...
  const [form, setForm] = useState<Partial<Aluno>>(INITIAL_FORM_STATE);
  ...
  const processarArquivoFoto = (arquivo: File) => {
    if (!arquivo.type.startsWith('image/')) {
      Swal.fire('Erro', 'Por favor, selecione um arquivo de imagem (JPG/PNG).', 'error');
      return;
    }

    const leitor = new FileReader();
    leitor.onloadend = () => {
      setForm(prev => ({ ...prev, foto: leitor.result as string }));
    };
  ...
  const handleUploadFoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (arquivo) processarArquivoFoto(arquivo);
  };

  const handleFotoArquivo = (e: React.ChangeEvent<HTMLInputElement>) => {
    const arquivo = e.target.files?.[0];
    if (arquivo) {
      if (!arquivo.type.startsWith('image/')) {
      ...
      const leitor = new FileReader();
      leitor.onloadend = () => {
        // USANDO setForm em vez de setAluno
        setForm(prev => ({ ...prev, foto: leitor.result as string }));
      };
      leitor.readAsDataURL(arquivo);
    }
  ...
  { !operador && <LoginOperador onLogin={setOperador} /> }
  ...
  const salvarOuAtualizarAluno = async () => {
    ...
    const alunoCompleto: Aluno = {
      ...(form as Aluno),
      documentos,
      operadorResponsavel: `${operador?.nome} - ${operador?.cargo}`,
      dataCriacao: new Date().toLocaleDateString('pt-BR'), // Garante que a data fique salva no registro
    };
  ...
  useEffect(() => {
    const operadorSalvo = localStorage.getItem("operador_sistema");
    if (operadorSalvo) {
      try {
        setOperador(JSON.parse(operadorSalvo));
      } catch (e) {
  ...
  // 3. Este useEffect roda SEMPRE que o operador mudar (Login ou Logout)
  useEffect(() => {
    if (operador) {
      localStorage.setItem("operador_sistema", JSON.stringify(operador));
    } else {
      localStorage.removeItem("operador_sistema");
    }
  }, [operador]);
  ...
  const estatisticas = useMemo(() => {
    ...
    if (totalAlunos === 0) return { total: 0, mediaIdade: 0 };

    const somaIdades = alunos.reduce((acc, aluno) => {
      // Cálculo de idade simples baseado na data de nascimento (AAAA-MM-DD)
      const hoje = new Date();
    ...
    return {
      total: totalAlunos,
      mediaIdade: (somaIdades / totalAlunos).toFixed(1)
    };
  }, [alunos]);

  return (
    <main className="container">
      {/* 1. Se não tiver operador, mostra APENAS o componente de login */}
      {!operador ? (
        <LoginOperador onLogin={setOperador} />
      ) : (
        /* 2. Se tiver operador, mostra o restante da aplicação */
        <>
          <div className="container">
            ...
            <div style={{
              ...
              <div className="estiloCard">
                <span style={{ fontSize: "14px", color: "#666" }}>Total de Alunos</span>
                <h2 style={{ fontSize: "32px", color: "var(--primary-blue)" }}>{estatisticas.total}</h2>
              </div>

              <div className="estiloCard">
                <span style={{ fontSize: "14px", color: "#666" }}>Média de Idade</span>
                <h2 style={{ fontSize: "32px", color: "var(--accent-orange)" }}>{estatisticas.mediaIdade} <small style={{ fontSize: "16px" }}>anos</small></h2>
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              ...
              <p style={{ color: "var(--text-light)", fontSize: "0.9rem" }}>
                Operador: <strong>{operador.nome}</strong> ( {operador.cargo} ) |
                <button
                  onClick={() => {
                    localStorage.removeItem("operador_sistema"); // Remove o item específico
                    setOperador(null); // Reseta o estado para voltar à tela de login
                  }}
                  style={{ border: "none", background: "none", color: "red", cursor: "pointer", textDecoration: "underline", marginLeft: "5px" }}
                >
                  Sair
                </button>
              </p>
            ...
            <section className="form-section">
              ...
              <div className="grid-2">
                <div className="input-block">
                  ...
                  <select
                    value={form.identidadeGenero === "Outro" || !["Cisgenero", "Homem Trans", "Mulher Trans", "Nao Binárie", "Gênero fluido", "Agênero", "Prefiro não informar", ""].includes(form.identidadeGenero || "") ? "Outro" : form.identidadeGenero}
                    onChange={e => {
                      const val = e.target.value;
                      setForm({ ...form, identidadeGenero: val });
                      if (val !== "Outro") setOutroGenero(""); // Limpa se mudar de ideia
                    }}
                  >
                    <option value="" disabled hidden>Selecione...</option>
                    <option value="Cisgenero">Homem Cisgênero</option>
                    <option value="Cisgenero">Mulher Cisgênero</option>
                    <option value="Homem Trans">Homem transgenero</option>
                    <option value="Mulher Trans">Mulher transgenero</option>
                    <option value="Nao Binárie">Não-binárie</option>
              ...
              {/* Verificamos se o valor é "Outro" OU se o valor salvo não está na lista padrão (caso de edição) */}
              {(form.identidadeGenero === "Outro" || (form.identidadeGenero && !["Cisgenero", "Homem Trans", "Mulher Trans", "Nao Binárie", "Gênero fluido", "Agênero", "Prefiro não informar", ""].includes(form.identidadeGenero))) && (
                <div className="input-block" style={{ marginTop: "10px" }}>
                  <label>Descreva sua identidade</label>
                  <input
                    type="text"
                    placeholder="Ex: Demigênero"
                    value={form.identidadeGenero === "Outro" ? outroGenero : form.identidadeGenero}
                    onChange={e => {
                      setOutroGenero(e.target.value);
            ...
            <section className="form-section">
              ...
              <div className="input-block">
                ...
                <select value={form.alunoEProprioResponsavel || ""} onChange={e => setForm({ ...form, alunoEProprioResponsavel: e.target.value as any })}>
                  <option value="" disabled hidden >Selecione...</option><option value="Sim">Sim</option><option value="Não">Não</option>
                </select>
              </div>
            ...
            <section className="form-section">
              ...
              <div className="grid-2">
                <div className="input-block">
                  <label>Mora em Àrea com Tratamento de Esgoto?</label>
                  <select value={form.temEsgoto || ""} onChange={e => setForm({ ...form, temEsgoto: e.target.value as any })}>
                    <option value="" disabled hidden >Selecione...</option><option value="Sim">Sim</option><option value="Não">Não</option>
                  </select>
                </div>
                <div className="input-block">
              ...
              <div className="grid-2">
                <div className="input-block">
                  ...
                  <select value={form.areaRiscoAmbiental || ""}
                    onChange={e => setForm({ ...form, areaRiscoAmbiental: e.target.value as any })}>
                    <option value="" disabled hidden >Selecione...</option>
                ...
                <div className="input-block">
                  ...
                  <select value={form.areaRiscoSeguranca || ""} onChange={e => setForm({ ...form, areaRiscoSeguranca: e.target.value as any })}>
                    <option value="" disabled hidden >Selecione...</option><option value="Sim">Sim</option><option value="Não">Não</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid-2">
                <div className="input-block">
                  ...
                  <select value={form.temFilhos || ""} onChange={e => setForm({ ...form, temFilhos: e.target.value as any })}>
                    <option value="" disabled hidden >Selecione...</option><option value="Sim">Sim</option><option value="Não">Não</option>
                  </select>
                </div>
                <div className="input-block">
                  ...
                  <input
                    ...
                    disabled={form.temFilhos !== "Sim"}
                    value={form.quantidadeFilhos || ""}
                    onChange={e => setForm({ ...form, quantidadeFilhos: parseInt(e.target.value) })}
                  />
                </div>
              </div>
              ...
              <div style={{ background: "#e8f8f5", padding: "15px", borderRadius: "8px", margin: "15px 0", border: "1px solid #d1f2eb" }}>
                ...
                {form.concluiuEnsinoMedio === "Não" && (
                  <div className="grid-2 anime-fade-in">
                    ...
                    <div className="input-block">
                      <label>Em qual tipo de escola?</label>
                      <select value={form.escolaPublica || ""} onChange={e => setForm({ ...form, escolaPublica: e.target.value as any })}>
                        ...
                        <option value="Privada com Bolsa">Privada com Bolsa</option>
                      </select>
                    </div>
                  </div>
                )}
              ...
              <div className="grid-2">
                <div className="input-block">
                  ...
                  <select value={form.temDeficiencia || ""} onChange={e => setForm({ ...form, temDeficiencia: e.target.value as any })}>
                    <option value="" disabled hidden >Selecione...</option><option value="Sim">Sim</option><option value="Não">Não</option>
                  </select>
                </div>
                <div className="input-block">
                  ...
                  <input
                    ...
                    disabled={form.temDeficiencia !== "Sim"}
                    value={form.qualDeficiencia || ""}
                    onChange={e => setForm({ ...form, qualDeficiencia: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="grid-2">
                ...
                <div className="input-block">
                  ...
                  <input
                    type="text"
                    placeholder="Ex: 220h/semana"
                    disabled={form.trabalha !== "Sim"}
                    value={form.cargaHorariaTrabalho || ""}
                    onChange={e => setForm({ ...form, cargaHorariaTrabalho: e.target.value })}
                  />
                </div>
              </div>
            ...
            <section className="form-section">
              ...
              <div className="input-block">
                ...
                <select value={form.tipoSanguineo || ""} onChange={e => setForm({ ...form, tipoSanguineo: e.target.value })}>
                  <option value="" disabled hidden >Selecione...</option><option value="A+">A+</option><option value="A-">A-</option><option value="B+">B+</option><option value="B-">B-</option><option value="AB+">AB+</option><option value="AB-">AB-</option><option value="O+">O+</option><option value="O-">O-</option><option value="Não Sabe">Não Sabe</option>
                </select>
              </div>
              ...
              <div className="grid-3">
                ...
                <div className="input-block">
                  <label>Telefone de Emergência 1</label>
                  <input
                    type="text"
                    placeholder="(XX) XXXXX-XXXX"
                    value={form.telefoneEmergencia1 || ""}
                    onChange={e => setForm({ ...form, telefoneEmergencia1: maskPhone(e.target.value) })}
                  />
                </div>
                <div className="input-block">
                  <label>Nome</label>
                  <input
                    ...
                    value={form.nomeEmergencia1 || ""}
                    onChange={e => setForm({ ...form, nomeEmergencia1: e.target.value })}
                  />
                </div>
                <div className="input-block">
                  ...
                  <input
                    type="text"
                    placeholder="Ex: Mãe, Tio..."
                    value={form.parentescoEmergencia1 || ""}
                    onChange={e => setForm({ ...form, parentescoEmergencia1: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="grid-3">
                ...
                <div className="input-block">
                  <label>Telefone de Emergência 2</label>
                  <input
                    type="text"
                    placeholder="(XX) XXXXX-XXXX"
                    value={form.telefoneEmergencia2 || ""}
                    onChange={e => setForm({ ...form, telefoneEmergencia2: maskPhone(e.target.value) })}
                  />
                </div>
                <div className="input-block">
                  <label>Nome</label>
                  <input
                    ...
                    value={form.nomeEmergencia2 || ""}
                    onChange={e => setForm({ ...form, nomeEmergencia2: e.target.value })}
                  />
                </div>
                <div className="input-block">
                  <label>Grau de Parentesco</label>
                  <input
                    ...
                    value={form.parentescoEmergencia2 || ""}
                    onChange={e => setForm({ ...form, parentescoEmergencia2: e.target.value })}
                  />
                </div>
              </div>
            </section>
            ...
            <section className="form-section">
              ...
              <div
                style={{
                  ...
                  gap: '20px',
                  padding: '20px',
                  border: isDragging ? '2px dashed var(--primary-blue)' : '2px dashed transparent',
                  ...
                  <div style={{ width: '100%', maxWidth: '300px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    >
                      <div style={{ textAlign: 'center', color: '#7f8c8d' }}>
                        ...
                        <strong>Tirar Foto via Webcam</strong>
                      </div>
                    </div>

                    {/* Botão para Upload (e área de soltar) */}
                    <button
                      type="button"
                      className="btn-secondary"
                      style={{
                        width: '100%',
                        backgroundColor: isDragging ? 'var(--primary-blue)' : '#34495e',
                        color: 'white',
                        border: 'none',
                        padding: '12px',
                        borderRadius: '8px',
                      ...
                      onClick={() => document.getElementById('upload-foto-input')?.click()}
                    >
                      📁 {isDragging ? "Solte a foto aqui!" : "Anexar Foto ou Arraste Aqui"}
                    </button>
                  </div>
                )}
            ...
            <ListaAlunos
              ...
              // CHAMADA SIMPLIFICADA AQUI:
              onDownloadAll={() => exportarLoteCompletoZip(alunos)}
